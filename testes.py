...
@app.route('/campanha_n/<int:id>',methods=['GET'])
@is_logged_in
def campanha_n(id):
        print(id)
        ...
        result = cursor.fetchall()
        print(f"SELECT *FROM campanha_question WHERE campanha = {id};")
        print(result)
        
        return render_template('campanha_n.html', columns_list = result, id=id)
...
def formacao_remota(type, id):
    ...
    if request.method == 'POST':
      ...
      if type == 'inquerito':
          cursor.execute(f"Insert into campanha_base values ({id},'{audio_intro}','{audio_con}','{audio_lingua}');" )
          conn.commit()

      else:
          cursor.execute(f"Insert into campanha_base values ({id},'{audio_intro}','{audio_con}','{audio_lingua}','{audio_base}');" )
          conn.commit()
    
      cursor.execute(f"update campanhas set projecto = '{project}' where id_campanha = {id} ;" )
      conn.commit()   
      conn.close()
...
def assign_question(id,type):
    ...
    return render_template('assign_question.html', form=form, type = type, id=id)
